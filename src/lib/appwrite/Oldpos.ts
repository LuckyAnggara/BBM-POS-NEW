...
export interface POSTransaction {
  ...
  discount: number
  totalAmount: number
  creditDueDate?: String | undefined
...
export async function getActiveShift(
  ...
  try {
    ...
    if (response.documents.length > 0) {
      ...
      // 1. Buat objek baru yang akan kita kembalikan, sesuai tipe POSShift.
      // 2. Petakan properti dari dokumen database (`doc`) ke objek baru.
      const mappedShift: POSShift = {
        id: doc.$id, // Paling penting: petakan `$id` dari database ke `id`
      ...
      // 3. Kembalikan objek baru yang sudah bersih dan benar bentuknya.
      return mappedShift

      // --- BAGIAN PERBAIKAN SELESAI ---
    }

    return null // Kembalikan null jika tidak ada dokumen yang ditemukan
  } catch (error) {

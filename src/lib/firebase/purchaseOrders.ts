...
import {
  ...
  runTransaction,
  increment,
} from 'firebase/firestore' // Added increment
...
export async function addPurchaseOrder(
  ...
  if (poData.items.length === 0)
    return { error: 'Pesanan pembelian harus memiliki minimal satu item.' }
  if (!poData.createdById)
    return { error: 'ID Pengguna pembuat PO diperlukan.' }
  ...
  try {
    ...
    const dataToSave: Omit<PurchaseOrder, 'id'> = {
      ...
      outstandingPOAmount:
        poData.paymentTermsOnPO === 'credit' ? totalAmount : 0,
      paymentStatusOnPO:
        poData.paymentTermsOnPO === 'credit'
          ? 'unpaid'
          : totalAmount > 0
          ? 'unpaid'
          : 'paid', // Adjusted for cash case with 0 total
...
export async function getPurchaseOrdersByBranch(
  ...
  let constraints: any[] = [where('branchId', '==', branchId)]
  ...
  if (options.orderByField) {
    ...
    constraints.push(
      orderBy(
        options.startDate && options.endDate ? 'orderDate' : 'createdAt',
        'desc'
      )
    )
  }
  ...
  try {
    await runTransaction(db, async (firestoreTransaction) => {
      ...
      if (
        ...
        throw new Error(
          `Tidak dapat menerima item untuk PO yang sudah ${
            purchaseOrder.status === 'fully_received'
              ? 'diterima penuh'
              : 'dibatalkan'
          }.`
      ...
      const productReadPromises = itemsReceived
        ...
        .map((recItem) => {
          ...
          return firestoreTransaction
            .get(itemRef)
            .then((snap) => ({ snap, recItem }))
        })

      const productSnapshotsWithReceivedData = await Promise.all(
        productReadPromises
      )
      ...
      for (const {
        snap: productSnap,
        recItem: receivedItem,
      } of productSnapshotsWithReceivedData) {
        if (!productSnap.exists()) {
          console.warn(
            `Produk dengan ID ${receivedItem.productId} tidak ditemukan di inventaris. Penerimaan untuk item ini dilewati.`
          )
          continue
        }
        ...
        if (poItemIndex === -1) {
          console.warn(
            `Item dengan productId ${receivedItem.productId} tidak ditemukan di PO ${poId}.`
          )
          continue
        }
        ...
        const newReceivedQuantity =
          poItem.receivedQuantity + receivedItem.quantityReceivedNow

        if (newReceivedQuantity > poItem.orderedQuantity) {
          throw new Error(
            `Jumlah diterima untuk ${poItem.productName} (${newReceivedQuantity}) melebihi jumlah dipesan (${poItem.orderedQuantity}).`
          )
        }
      ...
      for (const update of inventoryUpdatesForMutations) {
        ...
        firestoreTransaction.update(inventoryItemRef, {
          ...
          costPrice: purchaseOrder.items.find(
...
export async function recordPaymentToSupplier(
  poId: string,
  paymentDetails: Omit<PaymentToSupplier, 'paymentDate'> & {
    paymentDate: Date
    recordedByUserId: string
  }
): Promise<void | { error: string }> {
  ...
  try {
    ...
    const newOutstandingAmount =
      (purchaseOrder.outstandingPOAmount || 0) - paymentDetails.amountPaid
    let newPaymentStatus: PurchaseOrderPaymentStatus =

...
interface FirebaseBranchData
  extends Omit<Branch, 'id' | 'defaultReportPeriod'> {
  createdAt?: Timestamp
  updatedAt?: Timestamp
...
export async function createBranch(
  ...
  try {
    ...
    > & { createdAt: Timestamp } = {
      ...
      taxRate:
        branchData.taxRate === undefined ||
        branchData.taxRate === null ||
        isNaN(Number(branchData.taxRate))
          ? 0
          : Number(branchData.taxRate),
...
export async function updateBranch(
  ...
  const dataToUpdate: Partial<Omit<FirebaseBranchData, 'id' | 'createdAt'>> & {
    updatedAt: Timestamp
  } = {} as any
  if (updates.name?.trim()) dataToUpdate.name = updates.name.trim()
  if (updates.invoiceName !== undefined)
    dataToUpdate.invoiceName =
      updates.invoiceName.trim() || updates.name?.trim() || ''
  if (updates.currency?.trim()) dataToUpdate.currency = updates.currency.trim()
  if (
    updates.taxRate !== undefined &&
    updates.taxRate !== null &&
    !isNaN(Number(updates.taxRate))
  )
    dataToUpdate.taxRate = Number(updates.taxRate)
  if (updates.address !== undefined)
    dataToUpdate.address = updates.address.trim()
  if (updates.phoneNumber !== undefined)
    dataToUpdate.phoneNumber = updates.phoneNumber.trim()
  if (updates.transactionDeletionPassword !== undefined)
    dataToUpdate.transactionDeletionPassword =
      updates.transactionDeletionPassword
  if (updates.defaultReportPeriod)
    dataToUpdate.defaultReportPeriod = updates.defaultReportPeriod
...
export async function deleteBranch(
  ...
  try {
    ...
    if (!usersSnapshot.empty) {
      return {
        error: `Masih ada pengguna yang terhubung ke cabang ini. Hapus atau pindahkan pengguna terlebih dahulu.`,
      }
    }
    ...
    if (!itemsSnapshot.empty) {
      return {
        error: `Masih ada produk inventaris yang terhubung ke cabang ini. Hapus atau pindahkan produk terlebih dahulu.`,
      }
    }
    ...
    if (!transactionsSnapshot.empty) {
      return {
        error: `Masih ada data transaksi POS yang terhubung ke cabang ini.`,
      }
    }
    ...
    if (!bankAccountsSnapshot.empty) {
      return {
        error: `Masih ada data rekening bank yang terhubung ke cabang ini.`,
      }
    }

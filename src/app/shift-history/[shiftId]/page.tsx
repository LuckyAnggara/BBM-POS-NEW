...
import { getShiftById, getShiftTransactions } from '@/lib/laravel/shiftService'
...
export default function ShiftDetailPage() {
    ...
    try {
      ...
      if (fetchedShift) {
        setShift({
          ...fetchedShift,
          cash_difference:
            Number(fetchedShift.actual_balance || 0) -
            Number(fetchedShift.ending_balance || 0),
        })
      } else {
        toast({
  ...
  const fetchShiftTransactions = useCallback(async () => {
    ...
    try {
      const fetchedTransactions = await getShiftTransactions(shiftId)
      setTransactions(fetchedTransactions)
    } catch (error) {
      toast({
        title: 'Gagal Memuat',
        description: 'Gagal memuat transaksi shift.',
        variant: 'destructive',
      })
    } finally {
  ...
  }, [fetchShiftDetail, fetchShiftTransactions])

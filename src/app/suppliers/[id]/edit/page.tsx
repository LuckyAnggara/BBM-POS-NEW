...
import { type Supplier, type SupplierInput } from '@/lib/types'
import { getSupplier, updateSupplier } from '@/lib/laravel/suppliers'
...
export default function EditSupplierPage() {
  ...
  useEffect(() => {
    const fetchSupplier = async () => {
      ...
      try {
        ...
        supplierForm.reset({
          ...
          company_type: supplierData.company_type || 'company',
    ...
    fetchSupplier()
  }, [selectedBranch, supplierId, router, supplierForm])

  const onSubmitSupplier: SubmitHandler<SupplierFormValues> = async (
    values
  ) => {
    if (!selectedBranch || !supplier) {
    ...
    try {
      ...
      toast.error('Gagal Memperbarui Pemasok', {
          ...
          <form onSubmit={supplierForm.handleSubmit(onSubmitSupplier)}>
            ...
            <div className='flex justify-end gap-3 mt-6'>
              <Button type='button' variant='outline' asChild>
                <Link href={`/suppliers/${supplier.id}`}>Batal</Link>
              </Button>

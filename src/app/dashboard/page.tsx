...
export default function DashboardPage() {
  ...
  const [inventorySummary, setInventorySummary] = useState<InventorySummary | null>(null); // Inventory might not be date-range specific
  ...
  const fetchDashboardData = useCallback(async () => {
    if (!selectedBranch || !currentUser || !currentDisplayRange) {
      // Clear data if no branch or range
      ...
      // Inventory can be fetched independently if not date-range specific
      if (!selectedBranch) {
    ...
    setLoadingActiveShift(true); // Active shift is independent of date range, but fetch it along
    setLoadingChartSales(true);
    // Inventory is not typically date-range specific for summary, fetch once or less frequently
    if(loadingInventorySummary) { // Fetch inventory only if it's the initial load or branch changed
    ...
    try {
      const [
        ...
        getActiveShift(currentUser.uid, selectedBranch.id) // Active shift data
      ]);

      // Calculate dashboard stats for the selected range
      let netRevenue = 0;
      ...
      setDashboardStats({
        ...
        totalExpenses,
        netTransactionCount,
        revenueChangePercentage: "N/A", // Needs previous period data
        expenseChangePercentage: "N/A", 
        transactionChangePercentage: "N/A", 
      });
      setLoadingStats(false);

      // Process sales data for chart (daily for any selected range for now)
      const salesByDay: { [key: string]: { total: number, dateObj: Date } } = {};
      
      // Initialize days in the range for the chart
      let currentDate = startOfDay(start);
      while (currentDate <= endOfDay(end)) {
          const dateKey = format(currentDate, "yyyy-MM-dd");
          salesByDay[dateKey] = { total: 0, dateObj: new Date(currentDate) };
          currentDate = startOfDay(subDays(currentDate, -1)); // next day
      }
      ...
      const formattedChartSales = Object.entries(salesByDay)
        .map(([_, value]) => ({
            name: format(value.dateObj, "d MMM", { locale: localeID }), // e.g., "1 Jul", "2 Jul"
            total: value.total,
        }))
        .sort((a,b) => new Date(Object.values(salesByDay).find(d => d.dateObj && format(d.dateObj, "d MMM", { locale: localeID }) === a.name)?.dateObj || 0).getTime() - new Date(Object.values(salesByDay).find(d => d.dateObj && format(d.dateObj, "d MMM", { locale: localeID }) === b.name)?.dateObj || 0).getTime());


      setChartSalesData(formattedChartSales);
      setLoadingChartSales(false);

      // Calculate active shift summary (remains the same as it's current)
      if (activeShiftData) {
      ...
      setLoadingActiveShift(false);
      setLoadingChartSales(false);
      // setLoadingInventorySummary is handled separately
    }
  }, [selectedBranch, currentUser, currentDisplayRange, loadingInventorySummary]); // Added loadingInventorySummary

  useEffect(() => {
    if (selectedBranch && currentDisplayRange) { // Ensure currentDisplayRange is set before fetching
        fetchDashboardData();
    } else if (!selectedBranch) { // Clear data if no branch
        setDashboardStats(null);
        setInventorySummary(null);
        setActiveShiftSummary(null);
        setChartSalesData([]);
  ...
  }, [fetchDashboardData, selectedBranch, currentDisplayRange]); // Depends on currentDisplayRange now
    ...
    <ProtectedRoute>
      <MainLayout>
        <div className="space-y-4">
          ...
          {loadingActiveShift ? (
             <Skeleton className="h-28 w-full" /> // Skeleton for shift info card
          ) : activeShiftSummary && (
                  ...
                  <ChartContainer config={chartConfig} className="w-full h-full">
                    <BarChart accessibilityLayer data={chartSalesData} margin={{ top: 5, right: 0, left: -20, bottom: 5 }}>
                      ...
                      <XAxis
                        ...
                        tickMargin={8}
                        className="text-xs"
                        // tickFormatter={(value) => value.slice(0, 3)} // Use full date for longer ranges
                      />
                      <YAxis
                        tickLine={false}
                        axisLine={false}

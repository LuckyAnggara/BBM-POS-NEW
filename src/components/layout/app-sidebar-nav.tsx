...
import { navItemsConfig } from './menu'
...
export default function AppSidebarNav() {
  ...
  return (
    <nav className='flex flex-col h-full'>
      <SidebarHeaderBrand />
      <div className='flex-grow overflow-y-auto p-2 space-y-0.5 mt-1'>
        {' '}
        ...
        {loadingAuth || loadingUserData ? (
          ...
          <SidebarMenu>
            {' '}
            {/* UL as the root container for all items */}
            {navItems.map((item) => {
              if (item.adminOnly && userData?.role !== 'admin') {
                return null
              }

              const isCashierNoBranch =
                userData?.role === 'cashier' && !userData.branchId

              if (item.subItems && item.subItems.length > 0) {
                // This is an accordion group
                const isGroupDisabledForCashier =
                  isCashierNoBranch &&
                  !item.subItems?.some(
                    (sub) =>
                      sub.href === '/dashboard' ||
                ...
                const visibleSubItems = item.subItems.filter(
                  (subItem) =>
                    !(subItem.adminOnly && userData?.role !== 'admin')
                )
                if (
                ...
                const isGroupActive =
                  ...
                  ) && !isGroupDisabledForCashier

                return (
                  <SidebarMenuItem
                    key={item.label}
                    className='w-full my-0.5 p-0 list-none'
                  >
                    {' '}
                    {/* Wrapper LI */}
                    <Accordion type='single' collapsible className='w-full'>
                      <AccordionItem value={item.label} className='border-none'>
                        ...
                        <AccordionContent className='pt-0 pb-0 pl-5 pr-0 overflow-hidden'>
                          {' '}
                          {/* Indent sub-items */}
                          <SidebarMenuSub
                            className={cn(
                              '!mx-0 !px-0 !py-1 !border-l-0',
                              isGroupDisabledForCashier &&
                                'opacity-60 pointer-events-none'
                            )}
                          >
                            {visibleSubItems.map((subItem) => {
                              const isSubItemCashierNoBranch =
                                userData?.role === 'cashier' &&
                                !userData.branchId
                              const isSubItemAllowedForCashierNoBranch =
                                subItem.href === '/dashboard' ||
                                subItem.href === '/notifications' ||
                                subItem.href === '/account'
                              const isSubItemDisabled =
                                isSubItemCashierNoBranch &&
                                !isSubItemAllowedForCashierNoBranch
                              ...
                              return (
                                ...
                                >
                                  <Link
                                    href={
                                      isSubItemDisabled || !subItem.href
                                        ? '#'
                                        : subItem.href
                                    }
                                    passHref
                                  >
                                    <SidebarMenuSubButton
                                      ...
                                      className={cn(
                                        ...
                                        isSubItemActive
                                          ? 'text-primary font-medium bg-sidebar-accent/60'
              ...
              const isAllowedForCashierNoBranch =
                item.href === '/dashboard' ||
                item.href === '/notifications' ||
                item.href === '/account'
              const isNavItemDisabled =
                isCashierNoBranch && !isAllowedForCashierNoBranch
                  ...
                  >
                    <SidebarMenuButton
                      ...
                      className={cn(
                        ...
                        pathname === item.href && !isNavItemDisabled

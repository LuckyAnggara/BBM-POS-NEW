...
export default function ProtectedRoute({ children }: ProtectedRouteProps) {
  ...
  useEffect(() => {
    const intervalId = setInterval(() => {
      setCurrentMessageIndex(
        (prevIndex) => (prevIndex + 1) % loadingMessagesProtected.length
      )
    }, 2000)
  ...
  }, [currentUser, isLoadingUserData, router, pathname])

  // MODIFIED: Show full page loader only if currentUser is not yet determined AND isLoadingUserData is true.
